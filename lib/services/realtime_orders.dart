...
class RealTimeOrders {
  ...
  Future deleteOrder(String restaurant_id, String table_id) async {
    (await orders.doc(restaurant_id).collection(table_id).get())
        .docs
        .map((doc) {
  ...
  Future createOrderData(String restaurant_id, Order order) async {
    DocumentReference ref =
        orders.doc(restaurant_id).collection(actualTable).doc();
    bool found = false;
  ...
  Future updateOrderData(
      String restaurant_id, String table_id, Order order) async {
    DocumentReference ref =
        orders.doc(restaurant_id).collection(table_id).doc(order.order_id);
    await ref.set({
      'amount': order.amount,
      'entry_id': order.entry_id,
      'send': order.send,
